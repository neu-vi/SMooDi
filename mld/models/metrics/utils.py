...
def calculate_SRA(all_predicted, all_label):
    assert len(all_predicted) == len(all_label), "Arrays must have the same length"

    matches = np.sum(all_predicted == all_label)

    match_ratio =np.float64((matches / len(all_predicted)) * 100)
    return match_ratio
