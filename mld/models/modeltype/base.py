...
from mld.models.metrics import ComputeMetrics, MRMetrics, TM2TMetrics, MMMetrics, HUMANACTMetrics, UESTCMetrics, UncondMetrics, TM2TMetrics_Walk, TM2TMetrics_MST,TM2TMetrics_MST_XIA
    ...
    def configure_metrics(self):
        for metric in self.metrics_dict:
            if metric == "TemosMetric":
                ...
                self.TM2TMetrics_MST = TM2TMetrics_MST(
                    diversity_times=30
                    if self.debug else self.cfg.TEST.DIVERSITY_TIMES,
                    dist_sync_on_step=self.cfg.METRIC.DIST_SYNC_ON_STEP,
                )
            elif metric == "TM2TMetrics_MST_XIA":
                self.TM2TMetrics_MST_XIA = TM2TMetrics_MST_XIA(
                    diversity_times=30
                    if self.debug else self.cfg.TEST.DIVERSITY_TIMES,
                    dist_sync_on_step=self.cfg.METRIC.DIST_SYNC_ON_STEP,
                )
            elif metric == "TM2TMetrics_Walk":
                self.TM2TMetrics_Walk = TM2TMetrics_Walk(
                    diversity_times=30
                    if self.debug else self.cfg.TEST.DIVERSITY_TIMES,
                    dist_sync_on_step=self.cfg.METRIC.DIST_SYNC_ON_STEP,
                )
